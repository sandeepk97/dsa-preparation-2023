...
93
	 */
	public void sumOfAllIntegers(int[] arr) {
		long sum = 0;
		for (int num: arr)
			sum += num;
...
50
	 */
	public void elemAtIndex(int[] arr, int index) {
 ...
 */	
	...
	public int findNoOfElemsLessThan(int[] arr, int target) {
...
1 3 5
	 */	
		...
		public void printAlternateElems(int[] arr) {
			for (int i=0; i< arr.length; i++) {
				if (i % 2 == 0)
...
Explanation: Here Arr[1] = 1 exists.
	 */
	 ...
	 public int findIndexEqToElement(int[] arr) {
...
Output : NOT PERFECT
	 */
	 ...
	 public String isArrayPerfect(int[] arr) {
		for (int i=0; i<= arr.length/2; i++) {
			if (arr[i] != arr[n-i-1])
				return new String("NOT PERFECT");
		}
...
4
	 */
	 ...
	 // #Doubt #TODO
	 public void findElemRemaining(int[] arr) {
 ...
 */
	...
	public long sumOfAllElemsTillN(int n) {
		return n* (n+1)/2;
	}
}
 ...
 */
 ...
 public String isNumberFascinating(int n) {

	String res = new String("" + n + "" + 2*n + "" + 3*n);
	for (int i=1;i < 9; i++) {
		if (!res.contains(""+ i))
			return new String("Not Fascinating");


	}
	return new String("Fascinating");
...
To make the array balanced you can add 2.
	 */
	 ...
	 public int minimumNeededToBeBalanced(int[] arr) {
